...
console.log(greeting); // "Salom, Ali $1 Salom, !" ❌ — $1 literal qoladi (string pattern'da capture group yo'q), $` oldingi qismga aylanadi
...
`toUpperCase()` va `toLowerCase()` method'lari har doim bir xil uzunlikdagi string qaytarmaydi. Ba'zi Unicode belgilar case conversion'da **expand** bo'ladi — bir belgi ikki yoki undan ortiq belgiga aylanadi. Eng mashhur misollar: German `ß` → `SS` (1 → 2), Turkish `İ` → `i̇` (locale-independent lowercase'da 1 → 2 code point, i + combining dot above), Greek `ΐ` → `Ϊ́` (uppercase'da 1 → 3 code point).
